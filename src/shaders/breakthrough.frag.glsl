...
void main(){
  ...
  vec3 paleGold=mix(vec3(1.0),vec3(1.0,0.87,0.56),finalGoldMix);
  vec3 warmWhite=mix(vec3(1.0),vec3(1.0,0.94,0.78),finalGoldMix*0.72);
  vec3 pearlGold=mix(vec3(1.0),vec3(1.0,0.90,0.67),finalGoldMix*0.88);
  float staticWarp=uStaticWarpAmount*(0.035*sin(p.x*1.7+p.y*1.1)+0.025*sin(p.x*-1.2+p.y*1.9+1.4));
  vec2 q1=(p+staticWarp)-vec2(-0.18,0.08);
  vec2 q2=(p-staticWarp*0.7)-vec2(0.22,-0.13);
  vec2 q3=(p+vec2(staticWarp*0.4,-staticWarp))-vec2(0.02,0.20);
  float spread=max(uFinalLayerSpread,0.2);
  float glow1=exp(-dot(q1*vec2(0.65,1.0),q1*vec2(0.65,1.0))*0.85/spread);
  float glow2=exp(-dot(q2*vec2(1.0,0.72),q2*vec2(1.0,0.72))*0.70/spread);
  float glow3=exp(-dot(q3*vec2(0.80,0.55),q3*vec2(0.80,0.55))*0.62/spread);
  float broadHorizontal=exp(-p.y*p.y*0.28/spread);
  float diffuseLight=0.38*glow1+0.30*glow2+0.24*glow3;
  diffuseLight+=broadHorizontal*uFinalEdgeLight;
  diffuseLight*=mix(1.0,1.0-uDarkGapStrength,uFinalSpatialVariation);
  vec3 emittedLight=paleGold*glow1*0.38+warmWhite*glow2*0.30+pearlGold*glow3*0.24;
  float gapFactor=mix(1.0,clamp(diffuseLight/0.92,0.0,1.0),uDarkGapStrength);
  emittedLight*=mix(1.0,gapFactor,uFinalSpatialVariation);
